...
本章探讨如何使用 MP3+G 格式的卡拉 ok 文件。文件从服务器下载到与显示设备(我的电视)相连的(小)电脑上。使用运行在 Linux 或 Windows 上的 Java Swing 应用选择文件。
...
许多人已经收集了相当多的 MP3+G 歌曲。在这一章中，你将考虑如何列出并播放它们，同时保存喜爱歌曲的列表。本章着眼于一个 Java 应用来执行这一点，这实际上只是标准的 Swing 编程。本章不考虑特殊的音频或卡拉 ok 功能。
