...
![A435426_1_En_6_Fig3_HTML.jpg](A435426_1_En_6_Fig3_HTML.jpg)

图 6-3。

Pulse Audio Manager client information

![A435426_1_En_6_Fig2_HTML.jpg](A435426_1_En_6_Fig2_HTML.jpg)

图 6-2。

Pulse Audio Manager device information

![A435426_1_En_6_Fig1_HTML.jpg](A435426_1_En_6_Fig1_HTML.jpg)
...
![A435426_1_En_6_Fig4_HTML.jpg](A435426_1_En_6_Fig4_HTML.jpg)
...
![A435426_1_En_6_Fig5_HTML.jpg](A435426_1_En_6_Fig5_HTML.jpg)
...
![A435426_1_En_6_Fig6_HTML.jpg](A435426_1_En_6_Fig6_HTML.jpg)
...
PulseAudio 的一个特殊优势是可以进行应用级混音。如果两个音频源写入同一个 PulseAudio 设备，音频将被混合到输出设备。`pavucontrol`可以使用 Playback 选项卡显示多个应用程序，显示当前正在混合的所有应用程序或所有流。每个流都可以单独控制其通道音量。
...
![A435426_1_En_6_Fig7_HTML.jpg](A435426_1_En_6_Fig7_HTML.jpg)
...
![A435426_1_En_6_Fig8_HTML.jpg](A435426_1_En_6_Fig8_HTML.jpg)
...
步骤 1-7 对于大多数应用程序来说都是常见的。将调用上下文状态回调来响应服务器中的更改。这些是状态变化，例如`PA_CONTEXT_CONNECTIN`、`PA_CONTEXT_SETTING_NAME`等等。与大多数应用程序相关的变化将是`PA_CONTEXT_READY`。这意味着应用程序可以在稳定状态下向服务器发出请求。

在第 8 步中，应用程序将设置自己的行为。这是通过为各种操作设置进一步的回调函数来实现的，比如列出设备或播放音频。
...
其中`c`是上下文，`sinklist_cb`是应用程序的回调，`NULL`是传递给回调的用户数据。
...
在下面的程序中，您通过从`stdin`中读取整数值并使用这些值来设置特定设备上的音量。这种循环最好在 PulseAudio 框架的单独线程中进行。我没有在这里介绍应用程序线程，而是使用了另一组 PulseAudio 调用，为 PulseAudio 主循环设置了一个单独的线程。这些呼叫如下:
...
程序`palist_clients.c`类似于程序`palist_devices.c`。主要区别在于，当上下文将状态更改为`PA_CONTEXT_READY`时，应用程序订阅`PA_SUBSCRIPTION_MASK_CLIENT`而不是`(PA_SUBSCRIPTION_MASK_SINK|PA_SUBSCRIPTION_MASK_SOURCE)`，订阅回调请求`pa_context_get_client_info`而不是`pa_context_get_source_info`。
