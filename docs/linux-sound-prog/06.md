...
*   伦纳德的《脉冲音频和Jack》( [`http://0pointer.de/blog/projects/when-pa-and-when-not.html`](http://0pointer.de/blog/projects/when-pa-and-when-not.html) )
...
例如，廉价的Karaoke 可以通过以下方式将麦克风的直通模块设置为扬声器:
...
Karaoke 文件由诸如`kmid`到`timidity`的Karaoke 播放器播放。这里有一个例子:
