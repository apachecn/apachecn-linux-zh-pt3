...
映射通常使用声音字体文件来完成。这有各种格式。初级的是`.sf2`格式( [`http://connect.creativelabs.com/developer/SoundFont/Forms/AllItems.aspx/`](http://connect.creativelabs.com/developer/SoundFont/Forms/AllItems.aspx/) )。有些合成器(如胆怯)也可以使用 Gravis 超声贴片，这是录制的真实乐器。
...
*   您可以在“胆怯++配置文件包 v2004/8/3”页面( [`http://timidity.s11.xrea.com/files/readme_cfgp.htm`](http://timidity.s11.xrea.com/files/readme_cfgp.htm) )找到声音字体列表

可能令人惊讶的是，使用不同的声音字体似乎对 CPU 的使用没有太大的影响。对于 FluidSynth 来说，它们在一首歌曲上使用大约 60%到 70%的 CPU。当然，它们听起来确实不同。

## 胆怯

胆怯是一个“软件声音渲染器(MIDI 音序器和 MOD 播放器)”。它的主页是 Maemo.org([`http://maemo.org/packages/view/timidity/`](http://maemo.org/packages/view/timidity/))。

胆怯可以用来播放 MIDI 文件，方法是在命令行上给出它们，就像这样:
...
胆怯使用的默认声音字体是 Gravis 超声波补丁，来自`/usr/share/midi/freepats/`目录。这些声音字体是许多乐器所缺少的，因此应该被另一种字体所取代，例如 FluidSynth 字体。在配置文件`/etc/timidity/timidity.cfg`中进行设置。

### 作为服务器的胆怯

胆怯也可以作为监听端口的 ALSA 服务器运行(参见 [`http://wiki.winehq.org/MIDI`](http://wiki.winehq.org/MIDI) “在 UNIX 上使用 MIDI”)。
...
Midi 直通端口没有用，但是胆怯端口有用。然后，MIDI 文件可以由 ALSA 音序器播放，如下所示:
...
您可以使用`-O`选项更改胆怯的默认输出。胆怯帮助(`timidity -h`)显示如下内容:
...
胆怯可通过使用`-Oj`选项的插孔输出运行。在 Ubuntu 等基于用户的环境中，可能需要停止或暂停 PulseAudio，启动 Jack 服务器，然后运行 Timothy。例如，在一个终端中，可以通过以下方式暂停 PulseAudio:
