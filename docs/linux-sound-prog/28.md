...
胆怯是 MIDI 播放器，不是卡拉 ok 播放器。它被设计成一个具有特殊可扩展性的独立应用。开箱后，它可以播放卡拉 ok，但不是很好。这一章着眼于如何与胆怯建立一个卡拉 ok 系统。
...
另一种方法是构建一个主程序，将胆怯作为一个库。胆怯的命令行参数必须作为硬编码参数包含在应用中。一个简单的方法是:`CtlMode`有一个字段`trace_playing`，将它设置为 1 可以打开跟踪。将文本事件作为歌词事件需要更深入地挖掘胆怯，但只需要(在初始化库后不久)以下内容:
