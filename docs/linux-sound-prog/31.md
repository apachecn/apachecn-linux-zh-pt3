...
RPi 2 上TiMidity的 CPU 平均占 50 %, RPi 3 上TiMidity的 CPU 平均占 38%。如果其他应用(如 GUI 前端)也在使用，这可能会使它不可用。
...
像 DumpSequence 这样的程序工作正常。但是 SimpleMidiPlayer 达到了 80%的 CPU 使用率，是不可用的。因此，任何Karaoke 播放器在 RPi 上使用 Java 的想法都是不好的。树莓派网站上有一个讨论声音问题的帖子( [`www.raspberrypi.org/phpBB3/viewtopic.php?f=38&t=11009`](http://www.raspberrypi.org/phpBB3/viewtopic.php?f=38&t=11009) )。
