...
PyKaraoke 是用 Python 写的专用 Karaoke 播放器，使用了 Pygame、WxPython 等多种库。它会播放歌曲并显示你在歌词中的位置。一个“烟雾进入你的眼睛”( [`www.midikaraoke.com/cgi-bin/songdir/jump.cgi?ID=1280`](http://www.midikaraoke.com/cgi-bin/songdir/jump.cgi?ID=1280) )的屏幕截图看起来像图 [23-3](#Fig3) 。
...
是 KDE 的一名 Karaoke 歌手。它会播放歌曲并显示你在歌词中的位置。一个“烟雾进入你的眼睛”的屏幕截图看起来像图 [23-4](#Fig4) 。
