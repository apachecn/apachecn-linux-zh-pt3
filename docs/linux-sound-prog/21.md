...
胆怯被设计成一个独立的应用。为此，您应该构建一个新的“接口”它也可以被颠覆，就好像它是一个可以被调用的库。本章解释了这两种方式。

## 胆怯设计

胆怯被设计成一个独立的应用。当它被构建时，你得到了一个可执行文件，但是没有一个可以被调用的函数库，不像 FluidSynth。
...
这并不总是适合我，因为我更喜欢能够以一种简单的方式将胆怯嵌入到我自己的应用中。本章的其余部分从两方面来看。
...
一个由应用控制的系统在一个受管理的环境中工作得不是很好，比如 Windows(或者更近一些的环境，比如 Android)。在这样的环境中，你不能调用胆怯的`main`，而是调用属于框架的`main`函数。这将依次调用应用中的适当函数。
...
这是一个钩子，你需要把胆怯从一个独立的应用变成能够被另一个应用调用的库。请注意，您不能仅仅从构建中删除`timidity/timidity.c`。该文件包含太多其他关键功能！
...
真正胆怯的应用被编码为在许多不同的操作系统上使用许多不同版本的库。如前所述，在构建目标文件和库的过程中，大部分依赖项都被考虑进去了。
...
`compile`命令需要引入胆怯库和任何其他需要的库，用于 ALSA 应用。
...
前面的部分播放了 MIDI 和背景视频，本质上是作为独立的应用，作为独立的非交互线程。胆怯允许一个用户界面的更大集成，可以动态地添加到胆怯中。
...
胆怯不是为其他应用设计的。要么你添加一个新的接口，要么你绕开胆怯的设计去生产一个库。本章展示了这两种机制，并用简单和更复杂的例子进行了说明。