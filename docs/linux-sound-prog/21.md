...
胆怯被设计成一个独立的应用程序。为此，您应该构建一个新的“接口”它也可以被颠覆，就好像它是一个可以被调用的库。本章解释了这两种方式。

## 胆怯设计

胆怯被设计成一个独立的应用程序。当它被构建时，你得到了一个可执行文件，但是没有一个可以被调用的函数库，不像 FluidSynth。

胆怯你能做的就是增加不同的接口。例如，有 ncurses、Xaw 和哑接口可以在运行时调用，例如:
...
![A435426_1_En_21_Fig1_HTML.jpg](A435426_1_En_21_Fig1_HTML.jpg)

图 21-1。

TiMidity with Xaw interface

这个想法似乎是，如果你想要额外的东西，也许你应该建立一个自定义界面，并驱动它从胆怯。

这并不总是适合我，因为我更喜欢能够以一种简单的方式将胆怯嵌入到我自己的应用程序中。本章的其余部分从两方面来看。

*   将胆怯转化为一个库，并将其包含在您自己的代码中
*   构建您自己的界面

## 把胆怯变成图书馆

胆怯不是被设计成一个库，所以你必须说服它。这并不难；你只需要摆弄一下构建系统。

### 托管环境挂钩

一个由应用程序控制的系统在一个受管理的环境中工作得不是很好，比如 Windows(或者更近一些的环境，比如 Android)。在这样的环境中，你不能调用胆怯的`main`，而是调用属于框架的`main`函数。这将依次调用应用程序中的适当函数。

要使用这样的钩子，你需要下载胆怯的源代码，要么从包管理器，要么从胆怯++网站( [`http://timidity.sourceforge.net/`](http://timidity.sourceforge.net/) )。
...
如果从顶级源目录按以下方式构建胆怯，将产生一个错误，即`main`函数未定义:
...
这是一个钩子，你需要把胆怯从一个独立的应用程序变成能够被另一个应用程序调用的库。请注意，您不能仅仅从构建中删除`timidity/timidity.c`。该文件包含太多其他关键功能！

### 建造图书馆

要将胆怯构建为一个静态库，移除前面所示的`main`函数，并尝试构建胆怯。我发现我还需要指定我想要使用的输出系统，比如 ALSA。
...
通过从胆怯源目录的顶部运行以下命令，可以将所有的目标模块收集到一个归档文件中:
...
因为您必须从源代码开始构建胆怯，所以在您尝试构建这个备选库版本之前，请检查它是否在正常模式下工作。通过这种方式，你可以发现你需要，比如说，`libasound-dev`库来使用 ALSA，在你被其他东西弄混之前！

### 图书馆入口点

用`ANOTHER_MAIN`构建的胆怯暴露了这些公共入口点:
...
真正胆怯的应用程序被编码为在许多不同的操作系统上使用许多不同版本的库。如前所述，在构建目标文件和库的过程中，大部分依赖项都被考虑进去了。

一个最小的应用程序只是在`my_main.c`中的库入口点周围包装你自己的`main`。
...
`compile`命令需要引入胆怯库和任何其他需要的库，用于 ALSA 应用程序。
...
我将代码分成了两个文件，一个使用 Gtk 播放视频，另一个播放胆怯库并调用视频。视频播放文件为`video_code.c`。此处省略了代码，因为它本质上是第 [15 章](15.html)中描述的代码。

文件`video_player.c`设置胆怯环境，调用视频在后台播放，然后调用`play_midi`。内容如下:
...
前面的部分播放了 MIDI 和背景视频，本质上是作为独立的应用程序，作为独立的非交互线程。胆怯允许一个用户界面的更大集成，可以动态地添加到胆怯中。

### 共享对象

你可以构建自己的接口，添加到胆怯中，而不用改变或重新编译胆怯。这样的接口被构建为可动态加载的共享库，当胆怯开始时被加载。
...
胆怯只会加载以`if_`开头的文件。它们可以驻留在任何目录中，默认为类似于`/usr/lib/timidity`或`/usr/local/lib/timidity`(参见`timidity -h`中的“支持的动态加载接口”目录)。
...
这定义了关于接口和一组函数的信息，这些函数由胆怯调用，以响应胆怯内的事件和动作。例如，对于“我的界面”,该结构如下:
...
这太乱了。一个典型的接口将需要知道胆怯使用的一些常数和函数。虽然这些是为胆怯而有逻辑地组织起来的，但是对于一个新的界面来说，它们不是方便地组织起来的。所以，你必须不断地引入额外的包含，这些额外的包含指向其他外部的，需要更多的包含，等等。这些可能在不同的目录中，比如`timidity`和`utils`，所以你必须指向许多不同的包含目录。

请注意，您将需要胆怯的源代码来获得这些包含文件；你可以从 SourceForge 胆怯++下载( [`http://sourceforge.net/projects/timidity/?source=dlp`](http://sourceforge.net/projects/timidity/?source=dlp) )。

### 我的简单界面

这基本上和胆怯中内置的“哑”接口做的是一样的。它是从当前目录加载的，并通过以下命令调用:
...
下面的代码中有两个主要的函数，其余的都省略了。重要的功能有`ctl_event`和`ctl_lyric`。函数`ctl_event`处理胆怯产生的事件。对于这个接口，您只想在播放时打印歌词，所以当一个`CTLE_LYRIC`事件发生时，调用`ctl_lyric`。`ctl_lyric`功能使用胆怯功能`event2string`查找歌词，并将其打印到输出，如果需要，根据歌词文本打印换行符。接口文件如下所示:
...
当我试图使用标准包胆怯 v2.13.2-40.1 运行该接口时，它在一个内存释放调用中崩溃。代码被剥离了，所以追踪原因并不容易，我还没有费心去做——我不确定软件包发行版是针对什么库、代码版本等等进行编译的。

我从源头上建立了自己的胆怯的副本。这工作得很好。请注意，当您从源代码构建胆怯时，您需要指定它可以加载动态模块，例如，使用以下代码:
...
你可以从播放之前给出的视频中获取代码，并把它作为胆怯系统的“后端”作为“视频”接口。本质上所有需要做的就是从简单的接口改变`ctl_open`来调用 Gtk 代码播放视频，改变接口的身份。
...
胆怯不是为其他应用程序设计的。要么你添加一个新的接口，要么你绕开胆怯的设计去生产一个库。本章展示了这两种机制，并用简单和更复杂的例子进行了说明。