...
在我们的 CentOS 8 控制器上使用 Ansible 时，我们可以通过主机列表来定位我们想要直接管理的主机。该列表可作为选项提供给`ansible`命令。当然，我们可以做得更好。我们希望创建一个基于文件的持久主机列表，而不是临时列表。该列表是一个Ansible清单，在该文件中，我们还可以根据地理位置、功能或操作系统来定义组，这样就可以轻松地确定特定的目标主机。无论我们是直接在命令行中工作还是从剧本中工作，清单文件都提供了我们定位主机时所需的一致性。
