...
然而，过多的终端类型，出于所有的意图和目的，已经减少到一个单一的，标准的类型。这个标准，ISO 6429(又名 ECMA-48，原名 ANSI X3.64 或 VT100)，无处不在，不支持它的终端少之又少。因此，现在为单一终端类型编码是可行的。这种同质性的一个优点是必要的编码可以完全在Shell内完成。不需要外部命令。
